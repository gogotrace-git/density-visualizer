exports.handler = async function(event) {
  ...
  try {
    ...
    const prompt = [
      ...
      '字段要求：',
      '1. formula_expr 可以是 f(x,y)=... 这种形式，也可以只是表达式本身；但必须只保留二维密度函数的非零部分，绝对不要带“0, otherwise / 其余为0 / 其他情况为0”。',
      '2. formula_latex 填对应的 LaTeX 形式，格式类似 f(x,y)=e^{-y}。同样只保留非零部分，不要带分段函数中为 0 的部分。',
      '3. support 只填写非零部分对应的支持集/定义域，例如 0<x<y、0<=x<=1, 0<=y<=x。',
      '4. region 只有在图片中额外明确给出积分区域 D 时才填写。若图片中没有单独给出积分区域，就必须返回空字符串。不要把 support 误填到 region。',
      '5. source_text 只保留一行极简摘要，可为空字符串。不要重复完整分段函数，不要写长句解释。',
      '6. notes 只写极短备注，例如“图片模糊”“region 不清晰”；没有备注就置为空字符串。',
      '7. needs_review 只能是 true 或 false。看不清、有歧义时设为 true。',
      '8. 如果图片中给的是分段密度函数，例如 f(x,y)=e^{-y}, 0<x<y; 0, otherwise，那么你必须返回 formula_expr="f(x,y)=exp(-y)" 或 "exp(-y)"，support="0<x<y"，region=""。',
      '9. 不确定时宁可留空，不要猜测。',
      '返回格式示例：',
...
function cleanupRegion(value) {
  ...
  s = s.replace(/^区域[:：]?/, '');
  s = s.trim();
  if (looksLikeSupportText(s) && !/[Pp]\(|概率|积分/.test(s)) {
    return '';
  }
  return s;
}
